'''
Batch version using centroid
One centroid obtains multiple kmeans data at once, so there is no need to repeatedly process data

# Copied from attack-baseline: attack_baseline/hotflip_attack_yk_batch_ms_all.py
'''
...
def main():
    ...
    #     config=vars(args),
    # )
    # Cluster k at once, output multiple file_names simultaneously
    file_name_dic={}
    for k_s in range(args.k):
        ...
        args.num_iter, args.num_adv_passage_tokens,args.init_gold)  # Here args.kmeans_split is replaced with k_s

    device = torch.device('cuda' if torch.cuda.is_available() else "cpu")

    # Use one document to record all adversarial documents corresponding to each cluster
    output_dir = Path(f"output_attack/attacked_text/document/{args.attack_dataset}")
    output_dir.mkdir(parents=True, exist_ok=True)
    # Build filename with hyperparameters for easy experiment configuration identification
    file_name = (
        f"{args.attack_dataset}_train_attacked_documents_"
        f"seed_{args.seed}_"
        f"{args.attack_model_code}_supervised_hotflip_"
        f"N{args.k}_"  # Number of attack documents
        f"Len{args.num_adv_passage_tokens}_"  # Adversarial token length
        f"Iter{args.num_iter}_"  # Number of iterations
        f"Pool{args.num_cand}_"  # Candidate pool size
        f"Init{args.init_gold}" # Initialization method
        f".json"
    )
    ...
    # Load datasets
    # Different clusters are returned as a dictionary
    ...
    for k_s in range(args.k):
        ...
        for it_ in tqdm(range(args.num_iter), desc=f"Attacking..."):
            ...
            for _ in pbar: #Here _ is a placeholder, indicating that this variable is not needed, so no value is assigned. In fact, the for loop here will only be run once
                try:
                    ...
                    # Data exhausted, restart from beginning
                    train_iter_centrid_embedding = iter(train_loader)
                    it_centrid_embedding = next(train_iter_centrid_embedding)
        ...
        # Save adversarial results for all k clusters, overwrite each loop iteration
